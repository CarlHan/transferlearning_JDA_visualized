...
class knn:
    ...
    def fit_predict(self, Xs, Ys, Xt, Yt):
        ## ML model start
        clf = sklearn.neighbors.KNeighborsClassifier(n_neighbors=self.n_neighbors)
        clf.fit(Xs, Ys.ravel())
        Y_pre = clf.predict(Xt)
        acc = sklearn.metrics.accuracy_score(Yt, Y_pre)
        return acc


if __name__ == '__main__':
    domains = ['caltech.mat','dslr.mat','amazon.mat','webcam.mat']
    for i in range(1):
        for j in range(3):
            if i != j:
                ...
                _knn = knn(n_neighbors=1)
                acc_K = _knn.fit_predict(Xs, Ys, Xt, Yt)
                print('Non-transfer learning(NN):',acc_K)
                print('JDA transfer learning(NN):',acc_J)


    ##   separately
    src, tar = 'data/' + domains[1], 'data/' + domains[3]
    print(src)
    print(tar)
    src_domain, tar_domain = scipy.io.loadmat(src), scipy.io.loadmat(tar)
    Xs, Ys, Xt, Yt = src_domain['feas'], src_domain['label'], tar_domain['feas'], tar_domain['label']
